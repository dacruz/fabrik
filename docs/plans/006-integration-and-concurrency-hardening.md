...
- [x] Add root-level `integration_test` tests using `package integration_test` and only the public API; this package is the user-visible workflow proving the feature.
...
- [x] Run `go test ./...`, `go test -race ./...`, and suitable repeated stress runs; investigate and fix any race, deadlock, panic, or goroutine leak found.
- [x] Add concise public API and lifecycle documentation to package comments and update `README.md` with construction, subscribe, publish, unsubscribe, backpressure, and shutdown examples.
- [x] Verify the final file layout matches the spec and that tests assert behavior, not internal implementation details.

## Tests

- [x] Integration fan-out and topic isolation.
- [x] Backpressure with one slow and one healthy consumer.
- [x] Concurrent unsubscribe during active publishing.
- [x] Shutdown while publishers and consumers are active, including buffered drain.
- [x] Repeated race-detector and stress runs with no timing-dependent failures.
